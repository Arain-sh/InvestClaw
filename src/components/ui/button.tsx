...
const buttonVariants = cva(
  'inline-flex select-none items-center justify-center whitespace-nowrap rounded-full text-sm font-medium ring-offset-background transition-[background-color,color,border-color,box-shadow,transform] duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 active:scale-[0.985]',
  {
    variants: {
      variant: {
        default:
          'bg-primary text-primary-foreground hover:bg-primary/92 dark:bg-[#f4d586] dark:text-[#0f1712] dark:hover:bg-[#efd078]',
        destructive:
          'bg-destructive text-destructive-foreground hover:bg-destructive/90',
        outline:
          'border border-black/10 bg-white/86 text-foreground hover:bg-white hover:text-foreground dark:border-white/10 dark:bg-white/6 dark:hover:bg-white/9',
        secondary:
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    return (
      <Comp
        className={cn(buttonVariants({ variant, size, className }), 'shadow-none dark:shadow-none')}
        ref={ref}
        {...props}
      />
    );
