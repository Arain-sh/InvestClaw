...
<p align="center">
  <strong>OpenClaw ベースのデスクトップ投資リサーチ環境</strong>
</p>
...
**InvestClaw** は、OpenClaw のエージェントランタイムをコマンドラインなしで使える、投資リサーチ向けのデスクトップワークスペースです。相場確認、決算レビュー、ウォッチリスト管理、複数ステップの分析を 1 つの GUI にまとめます。

寄り前チェック、決算の読み解き、定期的なポートフォリオ確認などを、InvestClaw 上で整理して実行できます。

InvestClaw には OpenClaw 連携、複数モデルの設定、ドキュメント解析スキル、デスクトップ向け制御機能が含まれています。さらに **設定 → 詳細設定 → 開発者モード** から細かな調整も可能です。
...
InvestClawは公式の**OpenClaw**コアを直接ベースに構築されています。別途インストールを必要とせず、アプリケーション内にランタイムを組み込むことで、シームレスな「バッテリー同梱」体験を提供します。
...
`@agent` で別のエージェントを選ぶと、InvestClaw はデフォルトエージェントを経由せず、そのエージェント自身の会話コンテキストへ直接切り替えます。各エージェントのワークスペースは既定で分離されていますが、より強い実行時分離は OpenClaw の sandbox 設定に依存します。
...
InvestClaw には Tencent 公式の個人 WeChat チャンネルプラグインも同梱されており、Channels ページからアプリ内 QR フローで直接 WeChat を連携できます。
...
InvestClaw はドキュメント処理スキル（`pdf`、`xlsx`、`docx`、`pptx`）もフル内容で同梱し、起動時に管理スキルディレクトリ（既定 `~/.openclaw/skills`）へ自動配備し、初回インストール時に既定で有効化します。追加の同梱スキル（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`）も既定で有効化されますが、必要な API キーが未設定の場合は OpenClaw が実行時に設定エラーを表示します。  
...
InvestClawには、Electron、OpenClaw Gateway、またはTelegramなどのチャネルがローカルプロキシクライアントを介してインターネットにアクセスする必要がある環境向けに、組み込みのプロキシ設定が含まれています。
...
- InvestClawはTelegramが有効な場合、プロキシをOpenClawのTelegramチャネル設定にも同期します。
...
コミュニティからのコントリビューションを歓迎します！バグ修正、新機能、ドキュメントの改善、翻訳など、あらゆる貢献がInvestClawをより良くするのに役立ちます。
...
InvestClawは優れたオープンソースプロジェクトの上に構築されています：
...
### InvestClaw パートナープログラム 🚀

InvestClaw パートナープログラムを開始します。特に、カスタム AI エージェントや自動化ニーズを持つより多くの顧客に InvestClaw を紹介してくださるパートナーを募集しています。

パートナーの皆さまには、見込みユーザーや案件との接点づくりを担っていただき、InvestClaw チームは技術サポート、カスタマイズ、統合を全面的に提供します。

AI ツールや自動化に関心のある顧客とお仕事をされている方は、ぜひご一緒できればうれしいです。

詳細は [Issues](https://github.com/Arain-sh/InvestClaw/issues) または [Discussions](https://github.com/Arain-sh/InvestClaw/discussions) からご連絡ください。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=Arain-sh/InvestClaw&type=Date" alt="スター履歴チャート" />
</p>
