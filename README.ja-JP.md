...
チャット画面には IDE スタイルのリサーチデスクも追加され、右側の同じ可変幅サイドパネル内で `/workspace` のツリー型ファイルエクスプローラー、ファイルプレビュー、マルチタブの埋め込みマーケットブラウザを利用できます。ファイルプレビューでは `.html` をその場でレンダリングでき、単体の React `.tsx` / `.jsx` コンポーネントもワークスペース内で直接描画できます。
