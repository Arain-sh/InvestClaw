...
チャット画面には IDE スタイルの投資ターミナルも追加され、右側の同じ可変幅サイドパネル内で 3 カラムのリサーチデスクを利用できます。内容は `/workspace` のツリー型ファイルエクスプローラーとプレビュー、中央のネイティブ相場アプリ用ドック、そしてマルチタブの埋め込みマーケットブラウザです。ファイルプレビューでは `.html` をその場でレンダリングでき、単体の React `.tsx` / `.jsx` コンポーネントもワークスペース内で直接描画できます。内蔵ブラウザは、デスク内の他の操作を奪わないよう、必要になるまで読み取り専用表示のまま保たれます。
