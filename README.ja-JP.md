...
<p align="center">
  <a href="#機能">機能</a> •
  <a href="#なぜarainvestなのか">なぜAraInvestなのか</a> •
  <a href="#はじめに">はじめに</a> •
...
**AraInvest** は、コマンドラインなしで使える投資リサーチ向けのデスクトップワークスペースです。相場確認、決算レビュー、ウォッチリスト管理、複数ステップの分析を 1 つの GUI にまとめます。

寄り前チェック、決算の読み解き、定期的なポートフォリオ確認などを、AraInvest 上で整理して実行できます。

AraInvest にはランタイム機能、複数モデルの設定、ドキュメント解析スキル、デスクトップ向け制御機能が含まれています。さらに **設定 → 詳細設定 → 開発者モード** から細かな調整も可能です。

> AraInvest は調査支援ツールであり、投資助言を提供するものではありません。最終判断はご自身で確認してください。
...
投資リサーチは、散らかったタブや断片的なメモではなく、再現しやすいワークフローであるべきです。AraInvest は、エージェント駆動の分析を毎日の運用に落とし込みやすくするために作られています。

| 課題 | AraInvestのソリューション |
|------|----------------------|
...
AraInvest はランタイムをデスクトップアプリ本体に同梱しており、インストール、アップデート、日々の利用を 1 つの製品フローの中に収めています。
...
チャット画面には、現在の Agent 用の完全な右側 workspace エクスプローラーも追加されました。ディレクトリツリーの展開、ネストしたファイルのオープン、Markdown・HTML・テキスト・一般的な画像のインラインプレビューに対応しています。
`@agent` で別のエージェントを選ぶと、AraInvest はデフォルトエージェントを経由せず、そのエージェント自身の会話コンテキストへ直接切り替えます。各エージェントのワークスペースは既定で分離されていますが、より強い実行時分離はランタイムの sandbox 設定に依存します。
...
AraInvest には個人 WeChat 向けのチャンネルブリッジも同梱されており、Channels ページからアプリ内 QR フローで直接 WeChat を連携できます。
...
AraInvest はドキュメント処理スキル（`pdf`、`xlsx`、`docx`、`pptx`）もフル内容で同梱し、起動時に管理スキルディレクトリ（既定 `~/.openclaw/skills`）へ自動配備し、初回インストール時に既定で有効化します。追加の同梱スキル（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`）も既定で有効化されますが、必要な API キーが未設定の場合はランタイムが実行時に設定エラーを表示します。  
...
### 🌙 アダプティブテーマ
ライトモード、ダークモード、またはシステム同期テーマ。AraInvestはあなたの好みに自動的に適応します。

### 🚀 自動起動設定
**設定 → 通用** から **システム起動時に自動起動** を有効化すると、ログイン後に AraInvest が自動的に起動します。
...
AraInvestを初めて起動すると、**セットアップウィザード**が以下の手順をガイドします：
...
AraInvestには、Electron、AraInvest ゲートウェイ、または Telegram などのチャネルがローカルプロキシクライアントを介してインターネットにアクセスする必要がある環境向けに、組み込みのプロキシ設定が含まれています。
...
- 高度なプロキシフィールドが空の場合、AraInvestは`プロキシサーバー`にフォールバックします。
- プロキシ設定を保存すると、Electronのネットワーク設定が即座に再適用され、ゲートウェイが自動的に再起動されます。
- AraInvestはTelegramが有効な場合、プロキシをランタイムの Telegram チャネル設定にも同期します。
- AraInvestのプロキシが無効な状態では、Gatewayの通常再起動時に既存のTelegramチャネルプロキシ設定を保持します。
...
AraInvestは、**デュアルプロセス + Host API 統一アクセス**構成を採用しています。Renderer は単一クライアント抽象を呼び出し、プロトコル選択とライフサイクルは Main が管理します：
...
- AraInvest は Electron アプリのため、**1つのアプリインスタンスでも複数プロセス（main/renderer/zygote/utility）が表示される**のが正常です。
...
- ただし AraInvest ゲートウェイの待受は常に**単一**であるべきです。`127.0.0.1:18789` を Listen しているプロセスは1つだけです。
- Listen プロセスの確認例:
  - macOS/Linux: `lsof -nP -iTCP:18789 -sTCP:LISTEN`
  - Windows (PowerShell): `Get-NetTCPConnection -LocalPort 18789 -State Listen`
- ウィンドウの閉じるボタン（`X`）は既定でトレイへ最小化する動作で、完全終了ではありません。完全終了する場合はトレイメニューの **Quit AraInvest** を使用してください。
...
コントリビューションを歓迎します。バグ修正、新機能、ドキュメントの改善、翻訳など、あらゆる貢献がAraInvestをより良くするのに役立ちます。
...
AraInvestは[MITライセンス](LICENSE)の下でリリースされています。本ソフトウェアの使用、変更、配布は自由に行えます。
