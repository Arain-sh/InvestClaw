...
if /i "%1"=="update" (
    echo openclaw is managed by InvestClaw ^(bundled version^).
    echo.
    echo To update openclaw, update InvestClaw:
    echo   Open InvestClaw ^> Settings ^> Check for Updates
    echo   Or download the latest version from https://github.com/Arain-sh/InvestClaw/releases
    exit /b 0
)
...
set OPENCLAW_EMBEDDED_IN=InvestClaw
...
if "%_USE_BUNDLED_NODE%"=="1" (
    ...
    "%~dp0..\..\InvestClaw.exe" "%OPENCLAW_ENTRY%" %*
)
