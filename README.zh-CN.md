...
聊天主界面现在也加入了一个 IDE 风格的投资终端工作台，右侧会在同一个可调宽度的侧栏中提供一个统一研究空间，把 `/workspace` 树形文件浏览器与预览、原生行情/交易终端，以及支持多标签的内置市场浏览器合并到同一个自适应主区域里，通过视图切换来专注当前任务。现在在 `Apps` 视图里，支持网页入口的市场终端也可以直接嵌入到工作台画布中，不再强制额外跳到另一个界面，而且嵌入后的画布会采用更接近原生终端的壳层风格，而不是直接暴露浏览器感很强的顶部栏。文件预览区现在还能直接渲染 `.html` 文件，并在工作台里直接渲染独立的 React `.tsx` / `.jsx` 组件。内置浏览器默认会先以只读展示模式显示，需要时再显式激活交互，避免网页内容抢占文件系统和原生 App 的点击。
