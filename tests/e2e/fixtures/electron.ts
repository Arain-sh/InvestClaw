...
async function launchInvestClawElectron(homeDir: string, userDataDir: string): Promise<ElectronApplication> {
...
export const test = base.extend<ElectronFixtures>({
  homeDir: async ({ browserName: _browserName }, provideHomeDir) => {
    const homeDir = await mkdtemp(join(tmpdir(), 'investclaw-e2e-home-'));
  ...
  userDataDir: async ({ browserName: _browserName }, provideUserDataDir) => {
    const userDataDir = await mkdtemp(join(tmpdir(), 'investclaw-e2e-user-data-'));
    try {
  ...
  launchElectronApp: async ({ homeDir, userDataDir }, provideLauncher) => {
    await provideLauncher(async () => await launchInvestClawElectron(homeDir, userDataDir));
  },
