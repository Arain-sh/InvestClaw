; InvestClaw Custom NSIS Installer/Uninstaller Script
;
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting InvestClaw runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers InvestClaw.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; Even if InvestClaw.exe was not detected as running, orphan child processes
  ...
  ; PowerShell path-based kill may miss processes if the old InvestClaw was installed
  ...
  ; "Can't modify InvestClaw's files" retry loop -> "InvestClaw 无法关闭" dialog.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking "InvestClaw 无法关闭" dialog.
...
; "InvestClaw 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with InvestClaw's
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to remove InvestClaw application data?$\r$\n$\r$\nThis will delete:$\r$\n  • AppData\Local\investclaw (local app data)$\r$\n  • AppData\Roaming\investclaw (roaming app data)$\r$\n$\r$\nYour .openclaw folder (configuration & skills) will be preserved.$\r$\nSelect 'No' to keep all data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; Kill any lingering InvestClaw processes (and their child process trees) to
    ...
    RMDir /r "$LOCALAPPDATA\investclaw"
    RMDir /r "$APPDATA\investclaw"

    ; --- Retry: if directories still exist (locked files), wait and try again ---

    ; Check AppData\Local\investclaw
    IfFileExists "$LOCALAPPDATA\investclaw\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\investclaw"
      IfFileExists "$LOCALAPPDATA\investclaw\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\investclaw"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\investclaw
    IfFileExists "$APPDATA\investclaw\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\investclaw"
      IfFileExists "$APPDATA\investclaw\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\investclaw"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    StrCpy $R3 ""
    IfFileExists "$LOCALAPPDATA\investclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\investclaw"
    IfFileExists "$APPDATA\investclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\investclaw"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R3\AppData\Local\investclaw"
    RMDir /r "$R3\AppData\Roaming\investclaw"
