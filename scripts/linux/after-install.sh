...
# Post-installation script for InvestClaw on Linux
...
# Create symbolic link for InvestClaw app binary
if [ -x /opt/InvestClaw/investclaw ]; then
    ln -sf /opt/InvestClaw/investclaw /usr/local/bin/investclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/InvestClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/InvestClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/InvestClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/InvestClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/investclaw'
...
echo "InvestClaw has been installed successfully."
