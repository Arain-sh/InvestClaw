 ...
 */
export const PORTS = {
  /** InvestClaw GUI development server port */
  INVESTCLAW_DEV: 5173,
  
  /** InvestClaw GUI production port (for reference) */
  INVESTCLAW_GUI: 23333,

  /** Local host API server port */
  INVESTCLAW_HOST_API: 3210,
...
export function getPort(key: keyof typeof PORTS): number {
  const legacyKey = key.replace(/^INVESTCLAW_/, 'CLAWX_') as keyof typeof PORTS;
  const envValue = process.env[`INVESTCLAW_PORT_${key}`] ?? process.env[`CLAWX_PORT_${legacyKey}`];
...
export const APP_PATHS = {
  ...
  /** InvestClaw configuration directory */
  INVESTCLAW_CONFIG: '~/.investclaw',
  
  /** Log files directory */
  LOGS: '~/.investclaw/logs',
} as const;
