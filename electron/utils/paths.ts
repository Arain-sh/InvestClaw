...
function getElectronApp() {
  ...
  const fallbackUserData = process.env.INVESTCLAW_USER_DATA_DIR?.trim()
    || process.env.CLAWX_USER_DATA_DIR?.trim()
    || join(homedir(), '.investclaw');
...
/**
 * Get InvestClaw config directory
 */
export function getInvestClawConfigDir(): string {
  const preferred = join(homedir(), '.investclaw');
  const legacy = join(homedir(), '.clawx');
