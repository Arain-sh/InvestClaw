 ...
 * "openrouter.ai" carries the InvestClaw app-attribution headers.
 *
...
(function () {
  ...
  if (globalThis.__investclawFetchPatched) return;
  globalThis.__investclawFetchPatched = true;

  globalThis.fetch = function investclawFetch(input, init) {
    var url =
    ...
    if (url.indexOf('openrouter.ai') !== -1) {
      ...
      delete flat['x-title'];
      delete flat['X-Title'];
      flat['HTTP-Referer'] = 'https://github.com/Arain-sh/InvestClaw';
      flat['X-Title'] = 'InvestClaw';
      init.headers = flat;
    }
