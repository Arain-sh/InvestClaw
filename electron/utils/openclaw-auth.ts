 ...
 * Used by InvestClaw to seed the provider store when it's empty but providers are
...
/**
 * Write the InvestClaw gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  return withConfigLock(async () => {
    ...
    // Packaged InvestClaw loads the renderer from file://, so the gateway must allow
 ...
 * which means conversations disappear after roughly one day.  InvestClaw sets
 ...
 * OpenClaw/InvestClaw versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // When InvestClaw-managed moonshot provider exists, prefer centralized key
    ...
    // for InvestClaw to properly integrate with its updated tool system.
    ...
    // InvestClaw is a local desktop app where the user is the trusted operator.
    ...
    if (execConfig.security !== 'full' || execConfig.ask !== 'off') {
      ...
      toolsModified = true;
      console.log('[sanitize] Set tools.exec.security="full" and tools.exec.ask="off" to disable exec approvals for InvestClaw desktop');
    }
    ...
    if (typeof plugins === 'object' && !Array.isArray(plugins)) {
      ...
      // id="openclaw-qqbot" in its manifest, but older InvestClaw versions
    ...
    // but InvestClaw historically stored them only under `channels.<type>.accounts.default`.
