...
import { AimeSkillHubService } from '../gateway/aime-skillhub';
import { ensureInvestClawContext, repairInvestClawOnlyBootstrapFiles } from '../utils/openclaw-workspace';
...
const WINDOWS_APP_USER_MODEL_ID = 'com.arain.investclaw';
const isE2EMode = process.env.INVESTCLAW_E2E === '1' || process.env.CLAWX_E2E === '1';
const requestedUserDataDir = process.env.INVESTCLAW_USER_DATA_DIR?.trim() || process.env.CLAWX_USER_DATA_DIR?.trim();
...
// On Wayland this maps the running window to investclaw.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === 'linux') {
  app.setDesktopName('investclaw.desktop');
}
...
if (!gotElectronLock) {
  console.info('[InvestClaw] Another instance already holds the single-instance lock; exiting duplicate process');
  app.exit(0);
}
    ...
    if (!fileLock.acquired) {
      const ownerDescriptor = fileLock.ownerPid
        ...
        : fileLock.ownerFormat === 'unknown'
          ...
          : 'unknown owner';
      console.info(
        `[InvestClaw] Another instance already holds process lock (${fileLock.lockPath}, ${ownerDescriptor}); exiting duplicate process`,
      );
      app.exit(0);
    }
  } catch (error) {
    console.warn('[InvestClaw] Failed to acquire process instance file lock; continuing with Electron single-instance lock only', error);
  }
}
...
let aimeSkillHubService!: AimeSkillHubService;
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info('=== InvestClaw Application Starting ===');
  logger.debug(
  ...
  // Repair any bootstrap files that only contain InvestClaw markers (no OpenClaw
  // template content). This fixes a race condition where ensureInvestClawContext()
  // previously created the file before the gateway could seed the full template.
  if (!isE2EMode) {
    void repairInvestClawOnlyBootstrapFiles().catch((error) => {
  ...
  // Merge InvestClaw context snippets into the workspace bootstrap files.
  // The gateway seeds workspace files asynchronously after its HTTP server
  // is ready, so ensureInvestClawContext will retry until the target files appear.
  if (!isE2EMode) {
    void ensureInvestClawContext().catch((error) => {
      logger.warn('Failed to merge InvestClaw context into workspace:', error);
    });
  }
...
if (gotTheLock) {
  ...
  aimeSkillHubService = new AimeSkillHubService();
  hostEventBus = new HostEventBus();

  // When a second instance is launched, focus the existing window instead.
  app.on('second-instance', () => {
    logger.info('Second InvestClaw instance detected; redirecting to the existing window');
