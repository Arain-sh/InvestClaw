...
The chat workspace now also includes an IDE-style investment terminal on the right side with a resizable unified research desk. Files, native market apps, and the multi-tab market browser now share one adaptive workspace, so you can switch views without splitting attention across rigid columns. In the Apps view, web-capable market terminals can now be embedded directly inside the desk canvas instead of forcing an extra jump out to another surface, and the embedded surface now uses a native-terminal shell treatment rather than exposing obvious browser chrome. The file preview can render `.html` files inline and render standalone React `.tsx` / `.jsx` components directly inside the workspace pane. The built-in browser also stays in a safe read-only display mode until you explicitly activate page interaction, so embedded web content does not steal clicks from the rest of the desk.
