...
The chat workspace now also includes an IDE-style investment terminal on the right side with a resizable three-column research desk: a `/workspace` tree explorer and preview pane, a center dock for native market apps, and a multi-tab embedded market browser. The file preview can render `.html` files inline and render standalone React `.tsx` / `.jsx` components directly inside the workspace pane. The built-in browser also stays in a safe read-only display mode until you explicitly activate page interaction, so embedded web content does not steal clicks from the rest of the desk.
