...
The chat workspace now also includes an IDE-style research desk on the right side with a resizable `/workspace` tree explorer, file preview pane, and a multi-tab embedded market browser in the same side panel. The file preview can now render `.html` files inline and render standalone React `.tsx` / `.jsx` components directly inside the workspace pane.
