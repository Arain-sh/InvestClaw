...
The chat workspace now also includes an IDE-style investment terminal on the right side with a resizable unified research desk. Files, native market apps, and the multi-tab market browser now share one adaptive workspace, so you can switch views without splitting attention across rigid columns. In the Apps view, web-capable market terminals can now be embedded directly inside the desk canvas instead of forcing an extra jump out to another surface, and the embedded surface now uses a native-terminal shell treatment rather than exposing obvious browser chrome. The Apps dock also includes a compact quick-switch rail and in-canvas desk jumps so the flow feels closer to a native trading terminal than a browser wrapper. An AI command deck now sits inside the embedded market terminal itself, letting you inject market-pulse, earnings, trade-plan, and risk-radar prompts straight into the chat composer. The file preview can render `.html` files inline and render standalone React `.tsx` / `.jsx` components directly inside the workspace pane. The built-in browser also stays in a safe read-only display mode until you explicitly activate page interaction, so embedded web content does not steal clicks from the rest of the desk.
When you target another agent with `@agent`, InvestClaw switches into that agent's own conversation context directly instead of relaying through the default agent. Agent workspaces stay separate by default, and stronger isolation depends on runtime sandbox settings. The Agents page now lets you point each agent at a custom host workspace and inspect the files that will be mounted inside the sandbox at `/workspace`.
